...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (!empty($_POST['name2']) && !empty($_POST['phone2']) && !empty($_POST['date2'])) {
    ...
    if (isset($_POST['name2']) && !empty($_POST['name2'])) {
        $txt .= "Имя пославшего: " . strip_tags(trim(urlencode($_POST['name2']))) . "%0A";
    }
    ...
    if (isset($_POST['date2']) && !empty($_POST['date2'])) {
        $txt .= "Дата заезда: " . strip_tags(trim(urlencode($_POST['date2']))) . "%0A";
    }
