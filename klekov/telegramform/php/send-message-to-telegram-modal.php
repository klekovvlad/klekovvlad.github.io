...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (!empty($_POST['name1']) && !empty($_POST['phone1']) && !empty($_POST['date1'])) {
    ...
    if (isset($_POST['name1']) && !empty($_POST['name1'])) {
        $txt .= "Имя пославшего: " . strip_tags(trim(urlencode($_POST['name1']))) . "%0A";
    }
    
    // Номер телефона
    if (isset($_POST['phone1']) && !empty($_POST['phone1'])) {
        $txt .= "Телефон: " . strip_tags(trim(urlencode($_POST['phone1']))) . "%0A";
    }

    if (isset($_POST['date1']) && !empty($_POST['date1'])) {
        $txt .= "Дата заезда: " . strip_tags(trim(urlencode($_POST['date1']))) . "%0A";
    }
