...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (!empty($_POST['user_name']) && !empty($_POST['user_tel']) && !empty($_POST['user_date1']) && !empty($_POST['user_date2'])) {
    ...
    if (isset($_POST['user_name']) && !empty($_POST['user_name'])) {
        $txt .= "Имя пославшего: " . strip_tags(trim(urlencode($_POST['user_name']))) . "%0A";
    }
    
    // Номер телефона
    if (isset($_POST['user_tel']) && !empty($_POST['user_tel'])) {
        $txt .= "Телефон: " . strip_tags(trim(urlencode($_POST['user_tel']))) . "%0A";
    }

    if (isset($_POST['user_date1']) && !empty($_POST['user_date1'])) {
        $txt .= "Дата заезда: " . strip_tags(trim(urlencode($_POST['user_date1']))) . "%0A";
    }

    if (isset($_POST['user_date2']) && !empty($_POST['user_date2'])) {
        $txt .= "Дата выезда: " . strip_tags(trim(urlencode($_POST['user_date2']))) . "%0A";
    }
